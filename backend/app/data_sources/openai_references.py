...
from app.data_sources.us_market import YahooUSMarketValidator
...
class OpenAIReferenceResearcher:
    ...
    def __init__(self, api_key: str, *, model: str = "gpt-4o-mini", opener: Callable[..., Any] = urlopen, timeout_seconds: int = 45, us_market_validator: YahooUSMarketValidator | None = None) -> None:
    ...
    def research(self, theme: str, candidates: Sequence[DomesticCandidate]) -> ReferenceBundle:
        ...
        return _to_bundle(theme, parsed, sources, self._us_market_validator)


def _to_bundle(theme: str, value: dict[str, Any], sources: tuple[SourceRecord, ...], validator: YahooUSMarketValidator) -> ReferenceBundle:
    ...
    us_market = None
    if market.get("trend"):
        us_market = USMarketReference(theme, str(market["trend"]), str(market.get("background", "")), tuple(market.get("representative_companies", [])), tuple(market.get("representative_etfs", [])), str(market.get("news_summary", "")), _parse_date(market.get("as_of")), sources)
    peers = tuple(USPeerCompany(str(item["name"]), str(item["ticker"]), str(item["related_business"]), str(item["connection"]), "direct" if item.get("relevance") == "direct" else "indirect", sources) for item in value.get("peers", []) if item.get("name") and item.get("ticker") and validator.is_listed_equity(str(item["ticker"])))
    ...
    return ReferenceBundle(us_market, peers, managers)
...
def _schema() -> dict[str, Any]:
    ...
    return {"type": "object", "additionalProperties": False, "properties": {
        ...
        "peers": {"type": "array", "maxItems": 5, "items": {"type": "object", "additionalProperties": False, "properties": {"name": {"type": "string"}, "ticker": {"type": "string"}, "related_business": {"type": "string"}, "connection": {"type": "string"}, "relevance": {"type": "string", "enum": ["direct", "indirect"]}}, "required": ["name", "ticker", "related_business", "connection", "relevance"]}},
