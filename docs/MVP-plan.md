...
- `top_n`의 정수 여부와 최소값을 검증한다. 기본값 및 최대값은 미결정 사항으로 관리한다.
- 테마명과 국내 종목명 입력을 구분하고, 숫자만 입력·지나치게 포괄적인 표현·존재하지 않는 종목명은 수정 요청 대상으로 분류한다.
- 오류 시 분석을 시작하지 않고, 수정이 필요한 항목과 이유를 반환한다.

완료 조건: 정상 테마와 정상 국내 종목명이 통과하며, 빈 문자열·`top_n` 0 이하·문자열 `top_n`·식별 실패 입력이 명확한 오류를 반환한다.
...
- 완료 조건: 테마 입력 하나로 최대 3~5개의 국내 후보와 근거, 최근 종가·시가총액, 이슈, URL을 포함한 보고서가 생성된다.
